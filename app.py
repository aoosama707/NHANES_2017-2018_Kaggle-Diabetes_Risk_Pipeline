import os
import io
import base64
import joblib
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from flask import Flask, render_template, request, jsonify
from pathlib import Path


# -------------------------
# Optional SHAP
# -------------------------
try:
    import shap
    SHAP_AVAILABLE = True
except Exception:
    SHAP_AVAILABLE = False

# -------------------------
# CONFIG
# -------------------------
BASE_DIR = Path(__file__).resolve().parent
MODEL_PATH = BASE_DIR / "models" / "lr_tuned.joblib"

if not MODEL_PATH.exists():
    raise FileNotFoundError(f"Model not found at {MODEL_PATH}")

DEFAULT_FEATURES = [
    'age',
    'gender',
    'race',
    'bmi',
    'chol_total',
    'chol_hdl',
    'smoke_now',
    'family_diabetes',
    'physically_active',
    'bp_sys_mean',
    'bp_dia_mean'
]

# -------------------------
# APP
# -------------------------
app = Flask(__name__, template_folder="templates", static_folder="static")

# -------------------------
# LOAD MODEL
# -------------------------
saved = joblib.load(MODEL_PATH)

imputer = saved['imputer']
scaler = saved['scaler']
model = saved['lr_cal']   # calibrated logistic regression

FEATURE_NAMES = DEFAULT_FEATURES

# -------------------------
# HELPERS
# -------------------------
def preprocess(payload):
    row = {f: payload.get(f, np.nan) for f in FEATURE_NAMES}
    df = pd.DataFrame([row], columns=FEATURE_NAMES)
    df = df.apply(pd.to_numeric, errors='coerce')

    X_imp = imputer.transform(df)
    X_scaled = scaler.transform(X_imp)

    return X_scaled, df

# -------------------------
# ROUTES
# -------------------------
@app.route('/')
def home():
    return render_template("index.html")

@app.route('/predict', methods=['POST'])
def predict():
    try:
        # -------------------------
        # READ INPUT
        # -------------------------
        payload = {}
        for f in FEATURE_NAMES:
            val = request.form.get(f)
            try:
                payload[f] = float(val)
            except Exception:
                payload[f] = np.nan

        # -------------------------
        # PREPROCESS + PREDICT
        # -------------------------
        X_scaled, df_input = preprocess(payload)
        prob = float(model.predict_proba(X_scaled)[:, 1][0])

        # -------------------------
# SHAP VALUES (ROBUST FIX)
# -------------------------
        contributions = []
        shap_png = ""

        if SHAP_AVAILABLE:
            background = np.zeros((50, X_scaled.shape[1]))

        def predict_fn(x):
         return model.predict_proba(x)

        explainer = shap.KernelExplainer(predict_fn, background)
        shap_vals = explainer.shap_values(X_scaled, nsamples=100)

    # 🔴 FIX HERE
        if isinstance(shap_vals, list):
            shap_arr = np.array(shap_vals[-1]).reshape(-1)
        else:
            shap_arr = np.array(shap_vals).reshape(-1)

        contributions = list(zip(FEATURE_NAMES, shap_arr))
        contributions = sorted(contributions, key=lambda x: abs(x[1]), reverse=True)

    # -------------------------
    # CREATE SHAP PNG
    # -------------------------
        names = [x[0] for x in contributions]
        values = [x[1] for x in contributions]

        fig, ax = plt.subplots(figsize=(7, 4))
        ax.barh(names[::-1], values[::-1])
        ax.set_xlabel("SHAP value")
        ax.set_title("Feature Contributions")
        plt.tight_layout()

        buf = io.BytesIO()
        fig.savefig(buf, format="png", dpi=120)
        buf.seek(0)
        shap_png = base64.b64encode(buf.read()).decode("utf-8")
        plt.close(fig)

        # -------------------------
        # RENDER RESULT
        # -------------------------
        return render_template(
    "result.html",
    prob=prob,                     # ✅ send FLOAT
    contributions=contributions,
    shap_png=shap_png
        )


    except Exception as e:
        return f"Error: {str(e)}", 400

# -------------------------
# API (optional)
# -------------------------
@app.route('/api/predict', methods=['POST'])
def api_predict():
    data = request.get_json(force=True)
    X_scaled, _ = preprocess(data)
    prob = float(model.predict_proba(X_scaled)[:, 1][0])
    return jsonify({"probability": prob})

# -------------------------
# RUN
# -------------------------
if __name__ == "__main__":
    app.run(debug=True)
